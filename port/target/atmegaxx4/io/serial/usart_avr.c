...
void serial_avr_init(void)
{
    ...
    usart0.device.init = &usart_avr_init;
    usart0.device.puts = &usart_avr_puts;
    usart0.device.gets = &usart_avr_gets;
#ifdef SERIAL_INTERRUPT_MODE
    usart0.device.int_lock = &usart_avr_disable_interrupt;
    usart0.device.int_unlock = &usart_avr_enable_interrupt;
#endif /* SERIAL_INTERRUPT_MODE */
...
static int32_t usart_avr_gets(void *fd, void *priv_data, uint8_t *buf, int32_t nbytes, uint32_t flags)
{
    ...
    /* Disable USART interrupts. */
    usart_avr_disable_interrupt(&usart0);
