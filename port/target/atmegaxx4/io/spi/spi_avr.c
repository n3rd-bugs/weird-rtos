 ...
 * This function will initialize a AVR SPI device instance.
 */
void spi_avr_init(SPI_DEVICE *device)
{
    ...
    /* Update SPCR. */
    SPCR = (((device->cfg_flags & SPI_CFG_LSB_FIRST) != 0) << AVR_SPI_SPCR_DORD_SHIFT) |
    ...
    /* Enable SPI device. */
    SPCR |= (1 << AVR_SPI_SPCR_SPE_SHIFT);

} /* spi_avr_init */

/*
 * spi_avr_slave_select
 * This function will enable the slave so it can receive data.
 */
void spi_avr_slave_select(SPI_DEVICE *device)
{
...
/*
 * spi_avr_slave_unselect
 * This function will disable the slave.
 */
void spi_avr_slave_unselect(SPI_DEVICE *device)
{
    /* Remove some compiler warning. */
    UNUSED_PARAM(device);
 ...
 */
int32_t spi_avr_message(SPI_DEVICE *device, SPI_MSG *message)
{
    ...
    case SPI_MSG_WRITE:
        ...
        for (bytes = message->length; (bytes > 0); bytes -= this_len)
        {
            ...
            for (i = 0; i < this_len; i++)
            {
                ...
                timeout = 0;
                while((!(SPSR & (1 << SPIF))) && (timeout++ < AVR_SPI_TIMEOUT)) ;

                /* If we did not timeout while transferring data on SPI. */
                if (timeout >= AVR_SPI_TIMEOUT)
                {
