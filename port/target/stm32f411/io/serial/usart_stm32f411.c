...
static int32_t usart_stm32f411_init(void *);
static void usart_handle_tx_interrupt(STM32_USART *);
static void usart_handle_rx_interrupt(STM32_USART *);
static void usart_stm32f411_enable_interrupt(void *);
static void usart_stm32f411_disable_interrupt(void *);
static int32_t usart_stm32f411_puts(void *, void *, const uint8_t *, int32_t, uint32_t);
static int32_t usart_stm32f411_gets(void *, void *, uint8_t *, int32_t, uint32_t);
...
void serial_stm32f411_init(void)
{
    ...
    usart_stm32f411_register(&usart1, "usart1", 1, SERIAL_BAUD_RATE, &usart1_buffer_data, FALSE, TRUE);
#else
    usart_stm32f411_register(&usart1, "usart1", 1, SERIAL_BAUD_RATE, NULL, FALSE, TRUE);
#endif /* SERIAL_INTERRUPT_MODE */

} /* serial_stm32f411_init */

/*
 * usart_stm32f411_register
 ...
 */
int32_t usart_stm32f411_register(STM32_USART *usart, const char *name, uint8_t device_num, uint32_t baud_rate, FS_BUFFER_DATA *buffer_data, uint8_t hw_flow, uint8_t is_debug)
{
    ...
    case 1:
        ...
        GPIOA->AFR[1] |= (0x7 << ((9 % 8) << 2)) | (0x7 << ((10 % 8) << 2));
    ...
    case 2:
        ...
        RCC->APB1RSTR &= (uint32_t)~RCC_APB1RSTR_USART2RST;
        ...
        /* Select USART1 AF for PA2 and PA3. */
        GPIOA->AFR[0] &= (uint32_t)~((0xF << ((2 % 8) << 2)) | (0xF << ((3 % 8) << 2)));
        GPIOA->AFR[0] |= (0x7 << ((2 % 8) << 2)) | (0x7 << ((3 % 8) << 2));
    ...
    usart->serial.device.init = &usart_stm32f411_init;
    usart->serial.device.puts = &usart_stm32f411_puts;
    usart->serial.device.gets = &usart_stm32f411_gets;

    /* If we need to use interrupts for this USART. */
    if (buffer_data)
    {
        /* Hook-up interrupt locks. */
        usart->serial.device.int_lock = &usart_stm32f411_disable_interrupt;
        usart->serial.device.int_unlock = &usart_stm32f411_enable_interrupt;
...
/*
 * usart_stm32f411_init
 * @data: USART device data.
 * @return: Will always return success.
 * This function initializes USART1 for stm32f411.
 */
static int32_t usart_stm32f411_init(void *data)
{
 ...
 */
static int32_t usart_stm32f411_puts(void *fd, void *priv_data, const uint8_t *buf, int32_t nbytes, uint32_t flags)
{
    ...
    else
    {
        ...
        usart_stm32f411_disable_interrupt(usart);
 ...
 */
static int32_t usart_stm32f411_gets(void *fd, void *priv_data, uint8_t *buf, int32_t nbytes, uint32_t flags)
{
