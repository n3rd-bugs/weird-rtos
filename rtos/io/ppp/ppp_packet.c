...
int32_t ppp_packet_protocol_parse(FS_BUFFER_LIST *buffer, uint16_t *protocol, uint8_t pfc)
{
    ...
    if (buffer->total_length >= 1)
    {
        ...
        ASSERT(fs_buffer_list_pull(buffer, proto, 1, FS_BUFFER_INPLACE) != SUCCESS);
        ...
        else if (buffer->total_length >= 2)
        {
            ...
            ASSERT(fs_buffer_list_pull(buffer, &proto, 2, FS_BUFFER_INPLACE) != SUCCESS);

            /* First byte of protocol must be even and second must be odd. */
            if ((!(proto[0] & 0x1)) && (proto[1] & 0x1))
            {
                /* Pull the protocol field. */
                ASSERT(fs_buffer_list_pull(buffer, &ret_protocol, sizeof(uint16_t), FS_BUFFER_PACKED) != SUCCESS);
            }
...
int32_t ppp_packet_protocol_add(FS_BUFFER_LIST *buffer, uint16_t protocol, uint8_t pfc, uint8_t flags)
{
    ...
    status = fs_buffer_list_push(buffer, &(protocol), proto_len, (FS_BUFFER_PACKED | FS_BUFFER_HEAD | flags));
...
int32_t ppp_packet_configuration_header_parse(FS_BUFFER_LIST *buffer, PPP_CONF_PKT *packet)
{
    ...
    if (buffer->total_length >= 4)
    {
        ...
        ASSERT(fs_buffer_list_pull(buffer, &packet->code, 1, 0) != SUCCESS);
        ASSERT(fs_buffer_list_pull(buffer, &packet->id, 1, 0) != SUCCESS);
        ASSERT(fs_buffer_list_pull(buffer, &packet->length, 2, FS_BUFFER_PACKED) != SUCCESS);
...
int32_t ppp_packet_configuration_option_parse(FS_BUFFER_LIST *buffer, PPP_CONF_OPT *option)
{
    ...
    if (buffer->total_length >= 2)
    {
        ...
        ASSERT(fs_buffer_list_pull(buffer, &option->type, 1, 0) != SUCCESS);
        ASSERT(fs_buffer_list_pull(buffer, &option->length, 1, 0) != SUCCESS);
        ...
        if (option->length > 2)
        {
                ...
                ((uint32_t)(option->length - 2) <= PPP_MAX_OPTION_SIZE))
            {
                ...
                ASSERT(fs_buffer_list_pull(buffer, option->data, (uint32_t)(option->length - 2), 0) != SUCCESS);
            }
            else
            {
...
int32_t ppp_packet_configuration_header_add(FS_BUFFER_LIST *buffer, PPP_CONF_PKT *packet)
{
    ...
    status = fs_buffer_list_push(buffer, &packet->length, 2, (FS_BUFFER_PACKED | FS_BUFFER_HEAD));

    /* If length was successfully added. */
    if (status == SUCCESS)
    {
        /* Push configuration packet ID. */
        status = fs_buffer_list_push(buffer, &packet->id, 1, (FS_BUFFER_HEAD));
    }
    ...
    if (status == SUCCESS)
    {
        ...
        status = fs_buffer_list_push(buffer, &packet->code, 1, (FS_BUFFER_HEAD));
    }
...
int32_t ppp_packet_configuration_option_add(FS_BUFFER_LIST *buffer, PPP_CONF_OPT *option)
{
    ...
    status = fs_buffer_list_push(buffer, &option->type, 1, 0);

    /* If type was successfully added. */
    if (status == SUCCESS)
    {
        /* Add length for this option. */
        status = fs_buffer_list_push(buffer, &option->length, 1, 0);
    }
    ...
    if (option->length > 2)
    {
        ...
        status = fs_buffer_list_push(buffer, option->data, (uint32_t)(option->length - 2), 0);
    }
